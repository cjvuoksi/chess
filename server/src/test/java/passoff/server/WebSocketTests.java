...
public class WebSocketTests {
    ...
    private void makeMove(WebsocketUser sender, int gameID, ChessMove move, boolean expectSuccess,
        ...
        if (expectSuccess) {
            if (extraNotification)
                assertValidCommandMessages(actualMessages, sender, this::assertLoadGameWithExtra, inGame, this::assertMoveMadeWithExtra, otherClients);
            else
                assertValidCommandMessages(actualMessages, sender, this::assertLoadGameMessage, inGame, this::assertMoveMadePair, otherClients);
        } else assertInvalidCommandMessages(actualMessages, sender, inGame, otherClients);
    }
    ...
    private void resign(WebsocketUser sender, int gameID, boolean expectSuccess,
        ...
        if (expectSuccess)
            assertValidCommandMessages(actualMessages, sender, this::assertNotificationMessage, inGame, this::assertNotificationMessage, otherClients);
        else assertInvalidCommandMessages(actualMessages, sender, inGame, otherClients);
    }
    ...
    private void assertInvalidCommandMessages(Map<String, List<TestMessage>> messages, WebsocketUser user, Set<WebsocketUser> inGame, Set<WebsocketUser> otherClients) {
        ...
        for (WebsocketUser inGameUser : inGame)
            assertNoMessagesInvalid(inGameUser.username(), messages.get(inGameUser.username()));
        for (WebsocketUser otherUser : otherClients)
            assertNoMessagesFromOtherGame(otherUser.username(), messages.get(otherUser.username()));
    }
